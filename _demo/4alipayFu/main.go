...
// 初始化日志
func initLog() {
	f, _ := os.Create("./alipayFu_demo.log")
...
func giftRage(rate string) *[5]gift {
	...
	for i, data := range giftList {
		...
		giftList[i].rateMax = rateStart + giftList[i].rate
		if giftList[i].rateMax >= rateMax {
			giftList[i].rateMax = rateMax
			rateStart = 0
		} else {
			rateStart += giftList[i].rate
		}
	}
...
func saveLuckyData(code int32, id int, name, link, sendData string) {
	logger.Printf("lucky, code=%d, gift=%d, name=%s, link=%s, data=%s", code, id, name, link, sendData)
}
